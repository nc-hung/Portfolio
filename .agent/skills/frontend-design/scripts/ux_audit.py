...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # --- 1. PSYCHOLOGY LAWS ---
        # Hick's Law: Scoped to navigation elements if present
        nav_match = re.search(r'<nav[^>]*>(.*?)</nav>', content, re.DOTALL | re.IGNORECASE)
        nav_scope = nav_match.group(1) if nav_match else content
        nav_items = len(re.findall(r'<NavLink|<Link|<a\s+href|nav-item', nav_scope, re.IGNORECASE))
        if nav_items > 7:
            self.issues.append(f"[Hick's Law] {filename}: {nav_items} nav items in navigation (Max 7)")
